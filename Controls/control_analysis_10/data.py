...
class KineticsFrameDataset(Dataset):
    def __init__(
        self, data_dir, transform=None, split="train", class_names=None
    ):
        ...
        if class_names is not None:
            ...
            self.classes = sorted(
                [cls for cls in self.classes if cls in class_names]
            )
        ...
        for cls in self.classes:
            video_paths = glob.glob(
                os.path.join(data_dir, split, split, cls, "*.mp4")
            )
...
class KineticsFrameDataModule(pl.LightningDataModule):
    def __init__(
        self, data_dir, batch_size=64, num_workers=4, class_names=None
    ):
        super().__init__()
    ...
    def setup(self, stage=None):
        self.train_dataset = KineticsFrameDataset(
            self.data_dir,
            transform=self.transform,
            split="train",
            class_names=self.class_names,
        )
        self.val_dataset = KineticsFrameDataset(
            self.data_dir,
            transform=self.transform,
            split="val",
            class_names=self.class_names,
