...
for feature in feature_names:
    ...
    tick_labels = [int(tick * 100) for tick in ticks]  # Convert ticks to integers
    ...
    plt.axvline(x=10, color='lightgrey', linestyle='solid', linewidth=2, alpha=1)
    plt.axhline(y=50, color='lightgrey', linestyle='solid', linewidth=2, alpha=1)
    ...
    plotDir_svg = os.path.join(saveDir, f"time_gen_diff_{feature}_correlation.svg")
    plotDir = os.path.join(saveDir, f"time_gen_diff_{feature}_correlation.png")
    ...
    heatmap = ax.imshow(bool_time_reduced, cmap='jet', vmin = 0, vmax = 2, 
    ...
    plt.legend(handles=[significant_patch, non_significant_patch], loc='lower right', prop=legend_font_props)
    ...
    plotDir_svg = os.path.join(saveDir, f"time_gen_diff_{feature}_stats.svg")
    plotDir = os.path.join(saveDir, f"time_gen_diff_{feature}_stats.png")
    plt.savefig(plotDir_svg, format='svg', dpi=300, transparent=True)
    plt.savefig(plotDir, format='png', dpi=300, transparent=True)
    plt.close()