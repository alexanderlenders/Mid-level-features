...
if __name__ == "__main__":
    ...
    list_sub = (
        args.list_sub
    )  # list with subjects which should be included in the plot
    workDir_vid = (
        args.workdir_vid
    )  # directory with results of the time generalization analysis
    ...
    wholetg = (
        args.wholetg
    )  # whether to plot the whole time course (-400ms to 980ms) or only a selection, i.e. -200ms to 980ms
    ...
    for subject in list_sub_vid:
        if subject < 10:
            fileDir = (
                workDir_vid
                + "/Redone/decoding_miniclips_sub-0{}_redone.npy".format(
                    subject
                )
            )
        else:
            fileDir = (
                workDir_vid
                + "/Redone/decoding_miniclips_sub-{}_redone.npy".format(
                    subject
                )
            )
    ...
    for subject in list_sub_img:
        if subject < 10:
            fileDir = (
                workDir_img
                + "/Redone/decoding_images_sub-0{}_redone.npy".format(subject)
            )
        else:
            fileDir = (
                workDir_img
                + "/Redone/decoding_images_sub-{}_redone.npy".format(subject)
            )
    ...
    plt.savefig(
        plotDir_png, format="png", transparent=True, bbox_inches="tight"
    )
    plt.close()
