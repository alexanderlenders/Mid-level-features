...
if __name__ == "__main__":
    ...
    # Stats #
    ci_dir = os.path.join(
        statsDir, "encoding_{}_CI95_accuracy.pkl".format(input_type)
    )  # CI 95% encoding curve
    stats_dir = os.path.join(
        statsDir, "encoding_{}_stats_both_nonstd.pkl".format(input_type)
    )  # Permutation tests encoding curve
    ci_stats = os.path.join(
        statsDir,
        "encoding_{}_stats_peak_latency_CI_redone.pkl".format(input_type),
    )  # CI peak latency
    if input_type == "images" or input_type == "miniclips":
        peakDir = os.path.join(
            statsDir, "encoding_{}_CI95_peak.pkl".format(input_type)
        )  # CI 95% peaks
    elif input_type == "difference":
        peakDir = os.path.join(
    ...
    if input_type != "difference":
        if input_type == "images":
            noiseDir = (
                "Z:/Unreal/images_results/encoding/redone/noise_ceiling/"
            )
    ...
    plotDir = os.path.join(
        saveDir,
        "plot_encoding_full_7_features_{}_nonstd".format(input_type) + ".svg",
    )
    plt.savefig(plotDir, dpi=300, format="svg", transparent=True)
    plotDir = os.path.join(
        saveDir,
        "plot_encoding_full_7_features_{}_nonstd".format(input_type) + ".png",
    )
