...
if __name__ == "__main__":
    ...
    if input_type == 'miniclips':
        ...
        statsDir = 'Z:/Unreal/Results/Encoding/redone/stats/'

    elif input_type == 'images':     
        workDir = 'Z:/Unreal/images_results/encoding/'
        statsDir = 'Z:/Unreal/images_results/encoding/redone/stats/'
    ...
    # Stats #
    ci_dir = os.path.join(statsDir, 'encoding_{}_CI95_accuracy.pkl'.format(input_type)) # CI 95% encoding curve
    stats_dir = os.path.join(statsDir, 'encoding_{}_stats_both_nonstd.pkl'.format(input_type)) # Permutation tests encoding curve
    ci_stats = os.path.join(statsDir, 'encoding_{}_stats_peak_latency_CI_redone.pkl'.format(input_type)) # CI peak latency
    if input_type == 'images' or input_type == 'miniclips':
        peakDir = os.path.join(statsDir, 'encoding_{}_CI95_peak.pkl'.format(input_type)) # CI 95% peaks
    elif input_type == 'difference':
        peakDir = os.path.join(statsDir, 'encoding_diff_in_peak.pkl') # CI 95% peaks
    ...
    if input_type != 'difference':
        if input_type == 'images':
            noiseDir = ('Z:/Unreal/images_results/encoding/redone/noise_ceiling/')
        elif input_type == 'miniclips':
            noiseDir = ('Z:/Unreal/Results/Encoding/redone/noise_ceiling/')
        ...
        for s,subject in enumerate(list_sub):
            ...
            #lower
            fileDir_lower = (str(subject) + '_seq_50hz_posterior_lower_noise_ceiling.pkl')     
            ...
            #upper
            fileDir_upper = (str(subject) + '_seq_50hz_posterior_upper_noise_ceiling.pkl')     
    ...
    for i, feature in enumerate(sorted_features_mean):
        ...
        # Plot 
        ax[0].plot(timepoints, accuracy, color=sorted_color_dict[i], label=sorted_feature_names_graph[i], linewidth = 2) #accuracy
        ax[0].fill_between(timepoints, low_CI, high_CI, color=sorted_color_dict[i], alpha=0.3) #CIs
        ax[0].plot(timepoints, plot_accuracies, '*', color=sorted_color_dict[i], markersize=4)
        ...
        if input_type != 'difference':
            ax[0].plot([np.argmax(accuracy),np.argmax(accuracy)], [np.max(accuracy)-peak_tick_halflength,np.max(accuracy)+peak_tick_halflength], 
                    color='k', alpha=0.6, linestyle=':', linewidth=2)
    ...
    ax[0].set_xticks(ticks= x_tick_values_curves, labels=x_tick_labels_curves, fontsize=9, fontname=font, rotation = 45)
    ...
    ax[1].errorbar(x_pos, accuracies, yerr=[y_err_lower, y_err_upper], fmt='none', color='black',
                capsize=10, lw=2, elinewidth=2, ecolor='black', capthick=2)
    ...
    if input_type == 'miniclips' or input_type == 'images':
        ...
        ax[1].set_yticks(ticks = [0, 100, 200, 300, 400, 500, 600, 700, 800, 900], 
                        labels = [0, 100, 200, 300, 400, 500, 600, 700, 800, 900],
                        fontname=font, fontsize = 9)
    elif input_type == 'difference':
        ax[1].set_yticks(ticks = [0, 50, 100, 150, 200, 250, 300, 350, 400], 
    ...
    if input_type == 'images':   
        ...
        ax[0].legend(prop=legend_font_props, frameon=False,bbox_to_anchor=[0.6,0.82])
    ...
    # -----------------------------------------------------------------------------
    plt.show()
    plotDir = os.path.join(saveDir, 'plot_encoding_full_7_features_{}_nonstd'.format(input_type)  +'.svg')
    plt.savefig(plotDir, dpi = 300, format = 'svg', transparent = True)
    plotDir = os.path.join(saveDir, 'plot_encoding_full_7_features_{}_nonstd'.format(input_type)  +'.png')
