...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--sub", default=5, type=int, metavar="", help="subject number"
    )
    parser.add_argument(
        "-se",
        "--sess",
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="",
        help="Working directory: where EEG and behavioral data is stored",
    )
    parser.add_argument(
        "-p",
        "--plots",
    ...
    parser.add_argument(
        ...
        type=bool,
        metavar="",
        help="Save figures? Recommended only for ICA.\
                            Does only work, if sfigures = TRUE.",
    )
    parser.add_argument(
        "-i", "--ica", default=True, type=bool, help="Do an ICA?"
    )
    parser.add_argument(
        "-it",
        "--input_type",
...
def preprEeg(
    ...
    print(
        ">>> Preprocessing %dhz, sub %s, brain region: %s <<<"
    ...
    if input_type == "images":
        ...
        file_identifier = (
            "miniclip_vis_features_00"
            if sub < 10
            else "miniclip_vis_features_0"
        )

    if sub < 10:
        behavDir = workDir_input + (
            "/sub-0{}".format(sub) + "/beh/temporal_decoder_data"
        )
        eegDir = workDir_input + (
            "/sub-0{}".format(sub)
        ...
        behavDir = workDir_input + (
            "/sub-{}".format(sub) + "/beh/temporal_decoder_data"
        )
        eegDir = workDir_input + (
            "/sub-{}".format(sub)
            + "/eeg/{}{}".format(file_identifier, sub)
    ...
    events = events[
        np.logical_and(events[:, 2] != 10001, events[:, 2] != 10002)
    ]
    events = events[
        np.logical_and(events[:, 2] != 99999, events[:, 2] != 10003)
    ]
    ...
    identifiers = (
        []
    )  # this will contain the eeg ids (matching the behavioural ids)
    ...
    epochs = mne.Epochs(
        raw, seq_events, tmin=-1, tmax=6, baseline=None, preload=True
    )
    ...
    if do_ica:
        ...
        ica = mne.preprocessing.ICA(
            random_state=97, max_iter="auto", n_components=54
        )
        ...
        explained_variance = (
            np.square(mixing_matrix).sum(axis=0)
            / np.square(mixing_matrix).sum()
        )
        ...
        if show_plots:
            ...
            power_after_ica = epochs.plot_psd(
                tmax=np.inf, fmax=200, average=True
            )
    ...
    if show_plots:
        ...
        before_low_pass = before_downsampling.plot_psd(
            tmax=np.inf, fmax=250, average=True
        )
    ...
    if do_ica:
        if selected_channels == "posterior":
            sel_chan_regex = "^O *|^P *"
            chan_idx = mne.pick_channels_regexp(
                epochs.info["ch_names"], sel_chan_regex
            )
            chan_idx = np.asarray(chan_idx)
    ...
    for img_type in img_types:
        ...
        tmin = int(
            time_idx - 400 / (original_frequency / freq)
        )  # epoch_start(ms) / 1000 / resampling_freq(hz))
        tmax = int(
            time_idx + 1000 / (original_frequency / freq)
        )  # epoch_end(ms) / 1000 / resampling_freq(hz))
        ...
        for s in range(seq_data.shape[0]):  # for every sequence (out of 540)
            ...
            for i in range(
                img_per_seq
            ):  # for every stimulus in the sequence (out of 10)

                tmin = int(
                    time_idx - 400 / (1000 / freq)
                )  # epoch_start(ms) / 1000 / resampling_freq(hz))
                tmax = int(
                    time_idx + 1000 / (1000 / freq)
                )  # epoch_end(ms) / 1000 / resampling_freq(hz))
                ...
                base = np.mean(
                    img_data[
                        :, int(300 / (1000 / freq)) : int(400 / (1000 / freq))
                    ],
                    axis=1,
                )  # 100ms baseline ### should probably be (1000/freq)
        ...
        if show_plots:
            ...
            average_epoch, axes = matplotlib.pyplot.subplots(
                nrows=8, ncols=8, figsize=(30, 30)
            )
            matplotlib.pyplot.subplots_adjust(
                wspace=1, hspace=1, left=0.1, right=0.9, top=0.9, bottom=0.1
            )
            ...
            c1_each, axes = matplotlib.pyplot.subplots(
                nrows=3, ncols=1, figsize=(30, 30)
            )
            matplotlib.pyplot.subplots_adjust(
                wspace=1, hspace=1, left=0.1, right=0.9, top=0.9, bottom=0.1
            )
            ...
            # Create figure for each channel and subplots
            p1_each, axes = matplotlib.pyplot.subplots(
                nrows=7, ncols=1, figsize=(30, 30)
            )
            matplotlib.pyplot.subplots_adjust(
                wspace=1, hspace=1, left=0.1, right=0.9, top=0.9, bottom=0.1
            )
        ...
        if sub < 10:
            ...
            if not os.path.isdir(
                os.path.normpath(os.path.join(workDir_input, saveDir))
            ):
                os.makedirs(
                    os.path.normpath(os.path.join(workDir_input, saveDir))
                )
        ...
        if sub < 10:
            ...
            behav_dir = workDir_input + (
                "/sub-0{}".format(sub) + "/beh/" + "behavior_df.csv"
            )
            behavior_df.to_csv(behav_dir)
        else:
            behav_dir = workDir_input + (
                "/sub-{}".format(sub) + "/beh/" + "behavior_df.csv"
            )
            behavior_df.to_csv(behav_dir)
...
# -----------------------------------------------------------------------------
preprEeg(
    sub, sess, freq, select_channels, do_ica, workDir, show_plots, save_figures
)
