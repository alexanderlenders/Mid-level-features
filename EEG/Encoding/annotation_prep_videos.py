...
parser.add_argument('-vd', "--videos_dir", 
                    default = 'Z:/Unreal/frames/video',
                    type = str, metavar='', help="Video directory (9 image frames per video); update as needed")
...
def feature_extraction(videos_dir, annotations_dir, character_dir,
                       action_dir, save_dir, n_components, pca_method): 
    """
    ...
    def canny_edge(image, videos_dir, frame, openCV = True, gaussian_filter_size = 3):
        """        
        Parameters
        ----------
        image: int 
        ...
        """
        # Import image 
        image_file = str(image).zfill(4) + '_default_frame_{}'.format(frame) + '.jpg' # zfill: fill with zeros (4)
    ...
    def pca(features_train, features_val, features_test, pca_method = pca_method,
            n_comp = n_components, plot = False): 
        """        
        ...
        if feature == 'edges': 
            ...
            for video in range(num_videos): 
                ...
                for i, frame in enumerate(range(10, 19)): 
                    ...
                    features_flattened_frame = np.add(features_flattened_frame, feature_flatten)
                
                features_flattened_frame = np.divide(features_flattened_frame, num_frame)
            ...
            for video in range(num_videos): 
                ...
                for i, frame in enumerate(range(10,19)):
                    feature_np = skeleton_pos(video_index, annotations_dir, frame)
                    feature_flatten = feature_np.flatten()
                    features_flattened_frame = np.add(features_flattened_frame, feature_flatten)
            ...
            for video in range(num_videos): 
                ...
                for i, frame in enumerate(range(10, 19)): 
                    feature_np = world_normals(video_index, annotations_dir, frame)
                    feature_flatten = feature_np.flatten()
                    features_flattened_frame = np.add(features_flattened_frame, feature_flatten)
            ...
            for video in range(num_videos): 
                ...
                for i, frame in enumerate(range(10, 19)): 
                    feature_np = reflectance(video_index, annotations_dir, frame)
                    feature_flatten = feature_np.flatten()
                    features_flattened_frame = np.add(features_flattened_frame, feature_flatten)
            ...
            features_flattened = np.zeros((num_videos, len(actions)), dtype = float)
...
# STEP 3: Run Function
# -----------------------------------------------------------------------------
feature_extraction(videos_dir, annotations_dir, character_dir,
                       action_dir, save_dir, n_components, pca_method)
